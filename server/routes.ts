...
import { contactFormSchema, nurseries, galleryImages as galleryImagesTable, newsletters, events, insertReviewSchema } from "@shared/schema";
import { db } from "./db";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/reviews', async (req, res) => {
    try {
      ...
      if (nurseryId) {
        const reviews = await storage.getApprovedReviewsByNursery(parseInt(nurseryId as string));
        res.json(reviews);
      } else {
        const reviews = await storage.getAllReviews();
        res.json(reviews.filter(review => review.isApproved));
      }
    } catch (error) {
  ...
  app.get('/api/reviews/featured/:nurseryId', async (req, res) => {
    try {
      const nurseryId = parseInt(req.params.nurseryId);
      const reviews = await storage.getFeaturedReviewsByNursery(nurseryId);
      res.json(reviews);
    } catch (error) {
  ...
  // Admin Reviews API
  app.get('/api/admin/reviews', isAuthenticated, requireAnyAdmin, async (req, res) => {
    try {
      const reviews = await storage.getAllReviews();
      res.json(reviews);
    } catch (error) {
  ...
  app.get('/api/admin/reviews/pending', isAuthenticated, requireAnyAdmin, async (req, res) => {
    try {
  ...
  app.patch('/api/admin/reviews/:id/approve', isAuthenticated, requireAnyAdmin, async (req, res) => {
    try {
      const reviewId = parseInt(req.params.id);
      const success = await storage.approveReview(reviewId);
  ...
  app.patch('/api/admin/reviews/:id/feature', isAuthenticated, requireAnyAdmin, async (req, res) => {
    try {
      const reviewId = parseInt(req.params.id);
      const success = await storage.featureReview(reviewId);
      
      if (success) {
        res.json({ message: 'Review featured' });
      } else {
        res.status(404).json({ message: 'Review not found' });
      }
    } catch (error) {
  ...
  app.patch('/api/admin/reviews/:id/unfeature', isAuthenticated, requireAnyAdmin, async (req, res) => {
    try {
      const reviewId = parseInt(req.params.id);
      const success = await storage.unfeatureReview(reviewId);
      
      if (success) {
        res.json({ message: 'Review unfeatured' });
      } else {
        res.status(404).json({ message: 'Review not found' });
      }
    } catch (error) {
      console.error('Error unfeaturing review:', error);
      res.status(500).json({ message: 'Failed to unfeature review' });
    }
  });

  app.delete('/api/admin/reviews/:id', isAuthenticated, requireSuperAdmin, async (req, res) => {
    try {
      const reviewId = parseInt(req.params.id);
